      ...
      try {
        ...
        if (token) {
          ...
          if (isMounted && data.profile) {
            ...
            if (data.profile.phone || data.profile.condition) setAutofilled(true)
          }
        }
      } catch (e) {
        console.error("Autofill profile error:", e)
      }
    }
    loadSavedDetails()
    return () => {
        ...
        {step === "details" && (
          <div className="animate-fade-in">
            <div className="flex flex-wrap items-center justify-between gap-2 mb-4">
              <p className="text-sm text-slate-500 dark:text-slate-400">Your contact details</p>
              {autofilled && (
                <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-emerald-50 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800/60 shadow-xs">
