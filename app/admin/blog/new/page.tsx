...
import { Loader, Save, ArrowLeft, Upload } from "lucide-react"
...
export default function NewBlogPost() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const res = await fetch("/api/blog", {
      ...
      body: JSON.stringify({ title, slug, excerpt, content, image: imgUrl, youtubeUrl, category, keywords, author, createdAt: new Date().toISOString() }),
    })
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-navy-900 py-10 px-4">
      <div className="max-w-3xl mx-auto">
        ...
        <h1 className="text-2xl font-bold text-navy-800 dark:text-white mb-6">New Blog Post</h1>
        <form onSubmit={handleSubmit} className="bg-white dark:bg-navy-800 rounded-2xl p-6 border border-slate-200 dark:border-navy-700 shadow-xl space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" required value={title} onChange={(e) => { setTitle(e.target.value); autoSlug(e.target.value) }}
          ...
          <div>
            <label className="block text-sm font-medium text-navy-800 dark:text-white mb-1">Excerpt (short description)</label>
            <textarea required value={excerpt} onChange={(e) => setExcerpt(e.target.value)} rows={2}
              className="w-full px-4 py-2.5 rounded-xl border border-slate-300 dark:border-navy-600 bg-white dark:bg-navy-800 text-navy-800 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-brand-500" />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-navy-800 dark:text-white mb-1">YouTube URL (optional)</label>
          ...
          <div>
            ...
            <Editor value={content} onChange={setContent} placeholder="Start writing your blog post..." />
          </div>
